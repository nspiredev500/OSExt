...
void debug_shell_println_rgb(char* string,uint32_t r, uint32_t g,uint32_t b,...)
{
	...
	va_end(va);
	framebuffer_write10pstring_ascii(str,(void*) *LCD_UPBASE,0,line,r,g,b,ascii10p);
	line += 10;
	char *strpart = str+32;
	while (k_strlen(strpart,190) > 0)
	{
		framebuffer_write10pstring_ascii(strpart,(void*) *LCD_UPBASE,10,line,r,g,b,ascii10p);
		line += 10;
		if (line >= 240)
		{
			keypad_press_release_barrier(); // clear the screen after the user pressed a key to make place for new lines
...
void debug_shell_println(char* string,...)
{
	...
	sprintf_safe_va(str,string,180,va);
	va_end(va);
	framebuffer_write10pstring_ascii(str,(void*) *LCD_UPBASE,0,line,255,255,255,ascii10p);
	line += 10;
	char *strpart = str+32;
	while (k_strlen(strpart,190) > 0)
	{
		framebuffer_write10pstring_ascii(strpart,(void*) *LCD_UPBASE,10,line,255,255,255,ascii10p);
		line += 10;
		if (line >= 240)
		{
			keypad_press_release_barrier(); // clear the screen after the user pressed a key to make place for new lines
